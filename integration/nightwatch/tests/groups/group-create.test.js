module.exports = {
    ...
    "Can add new group with user as a member and member and admin role show up as expected"(browser) {
        ...
        groupDetail.assertGroupName(groupName).assertGroupMemberSize(1).assertGroupAdminSize(1).backToGroupList();
    ...
    "Can add new group with own provided ID"(browser) {
        ...
        groupCreate.enterGroupID(groupID).enterGroupName(groupName).submitNewGroup();

        demoApp.assertOnGroupDetailPage();

        groupDetail.assertGroupID(groupID).assertGroupName(groupName).assertGroupMemberSize(1).assertGroupAdminSize(1);
    ...
    "Can add new group with name not as a member and user does not exist in member list"(browser) {
        ...
        groupCreate.enterGroupName(groupName).enterGroupID(groupId).toggleAddAsMember().submitNewGroup();

        demoApp.assertOnGroupDetailPage();

        groupDetail.assertGroupName(groupName).assertGroupMemberSize(0).assertGroupAdminSize(1).backToGroupList();
